...
impl McpTool for FindDependentsTool {
    ...
    fn run(args: &Self::Args, state: &AppState) -> Result<ToolOutput, ToolError> {
        ...
        // substring match (ContractGraph::find_dependents' last-resort
        // fallback) can span multiple unrelated services that share a
        // locally-aliased package name, so the formatter groups by this
        // label instead of flattening everything into one list.
